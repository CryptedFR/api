...
test.group("Friendship accept", (group) => {
	let user1: User;
	let user2: User;
	let user3: User;
	...
	test("Should success when user 1 accept request", async ({
		client,
		assert,
	}) => {
		const response = await client
			.patch(`/v1/friendship/accept/${friend_request_id}`)
	...
	test("Should fail when user 1 try to accept request already accepted", async ({
		client,
		assert,
	}) => {
		const response = await client
	...
	test("Should fail when user 1 try to accept unknown request", async ({
		client,
		assert,
	}) => {
		const response = await client
			.patch("/v1/friendship/accept/23")
			.header("Authorization", `Bearer ${user_1_token}`);
	...
	test("Should fail when user 2 try to accept blocked request from user 3", async ({
		client,
		assert,
	}) => {
		const response = await client
			.patch(`/v1/friendship/accept/${blocked_request_id}`)
